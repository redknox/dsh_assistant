...
**Self-Extension** must use these same public seams. Governance is **Designed** ([docs/self-extension.md](./docs/self-extension.md)). The Capability Registry that answers **What do I have?** is **Verified** ([docs/capability-registry.md](./docs/capability-registry.md)). Capability Resolution Review that answers **What should change?** is **Verified** ([docs/capability-resolution.md](./docs/capability-resolution.md)). Candidate workspace and validation that answer **Can I build it safely?** are **Verified** ([docs/candidate-workspace.md](./docs/candidate-workspace.md)). Invariants: **self-extension without self-authorization**, and prefer reuse/evolution over new plugins. Assistant-generated code has no privileged runtime path. Registry `active` is metadata, not a mount. A resolution review is a proposal, not approval. `validated` is not permission to activate.
...
This document is **Implemented** as a contract. Runtime boot, personal services, the UI control surface, the Plan My Day slice, the DSH bundle/profile pack, the Capability Registry, Capability Resolution Review, and candidate workspace/validation are **Verified** by `npm test`. Scripted local LLM adapters and fake integration providers are **Implemented**. Self-Extension governance is **Designed** ([docs/self-extension.md](./docs/self-extension.md)). Live LLM accounts, vendor OAuth, pixel-perfect UI, RAG, production persistence, and autonomous install are **Unsupported**. See [docs/vertical-slice.md](./docs/vertical-slice.md), [docs/packaging.md](./docs/packaging.md), [docs/capability-registry.md](./docs/capability-registry.md), [docs/capability-resolution.md](./docs/capability-resolution.md), and [docs/candidate-workspace.md](./docs/candidate-workspace.md).
