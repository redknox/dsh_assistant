...
Target architecture for the personal assistant product layer on DeepSeek Harness (DSH). This issue records contracts only; runtime and UI are **Designed**, not Implemented.
...
**MVP (Designed):** compose on DSH public seams; keep domain independent of model/UI; separate memory, knowledge, tools, and policy; distinguish read / propose / execute; smallest useful product loop.

**Out of scope unless a later issue says otherwise (Unsupported here):** custom Agent Loop; speculative multi-agent framework; forced database; runtime/UI implementation in this documentation issue; credentials or real personal data in the repo.

## Evidence

This document is **Implemented** as a contract. Runtime layers, plugins, and UI are **Designed**. Nothing in this file is **Verified** until there is reproducible evidence (see [ENGINEERING.md](./ENGINEERING.md)).
