...
export function gatherWorkspaceSnapshot(input: GatherWorkspaceInput): WorkspaceSnapshotInput {
  ...
  return {
    ...
    integrationStatus: [
      ...Object.entries((ctx.get('integrations') as { hub: { status(): Record<string, { available: boolean; configured?: boolean; reason?: string; provider?: string; lastVerifiedAt?: string; authorization?: { state: 'ready' | 'expiring' | 'expired' | 'unavailable' } }> } } | undefined)?.hub.status() ?? {}),
      ...webIntegrationStatus(ctx),
    ]
      .map(([capability, availability]) => {
        const observed = availability as typeof availability & { lastVerifiedAt?: string; authorization?: { state?: 'ready' | 'expiring' | 'expired' | 'unavailable' } }
        const authorization = observed.authorization?.state
        return {
          capability,
          ...
          ...(observed.lastVerifiedAt ? { lastVerifiedAt: observed.lastVerifiedAt } : {}),
