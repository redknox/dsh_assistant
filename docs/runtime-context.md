...
Status: **Implemented**. This is the host-owned Profile / Workspace / Session contract carried into the current [v0.5.0 baseline](./v0.5.0-baseline.md). Multi-session topic lifecycle and Capability Delivery Sessions are **Implemented** in [session-catalog.md](./session-catalog.md). Governed local third-party import is a separate Self-Extension capability (`import-local`). Profile-scoped DSH Skill lifecycle is **Implemented** in [skills.md](./skills.md). Marketplace discovery is not Implemented here.
...
A Home whose binding still stores the Profile **name** as `profileIdentity` is upgraded under the Home lease **before** the final partition is claimed. Upgrade copies session files excluding `.writer.lock*`, then rewrites the Session Root owner, then rewrites the Home binding. The phase journal at `$HOME/state/profile-identity-migration.json` stores only schema, from/to identity, and phase; partition paths are always re-derived from the trusted Home binding and current Session Root. A missing journal is distinct from a malformed or future-schema journal, which fail closed. An interrupt leaves either the old-readable state or a journal that the next `start` can complete. `doctor` stays read-only while a journal is present. After a successful identity upgrade, the holding Home lease is atomically rewritten with the same run token so lease, binding, doctor, and Mission-Control share the resolved Profile identity before the Web UI is published.
