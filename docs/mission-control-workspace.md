...
Each projected capability also carries separate runtime, configuration, authentication, and data evidence. Feishu can therefore say `AUTH VERIFIED · DATA CHECKED ON USE`; a token-configured provider with no live probe says `CONFIGURED · VERIFIED ON USE`; Memory and Knowledge say `EMPTY` when their services are mounted but contain no current records. Context stores report `ACTIVE`, not `READY`. External development executors separate executable discovery, provider-account login, custom model-route configuration and verified execution. A Claude custom route can therefore be available without an Anthropic login, while still saying `VERIFY ON RUN`. Failed real-run authentication is retained across restart instead of reverting to a green installation lamp.
...
The current conversation may also project a read-only **Session Work Context** above its message history. For an ordinary DSH Goal it shows the Goal objective and phase. For a Capability Delivery Session it joins the persisted Proposal, current immutable Specification revision, Resolution Plan, Candidate, approval, and active Registry evidence into one user-facing stage rail. This projection owns no lifecycle state and cannot advance delivery; it makes the Harness's current context durable and legible without creating a second orchestrator.

Starting an accepted Capability Proposal creates the dedicated Session first, binds the Proposal to that trusted Session id, and then creates one native DSH Goal in the same Session. Before every model step, a dynamic context re-reads that Goal and the authoritative Workbench delivery stage; retained chat text is not the source of the objective. The Goal round driver remains dynamic, but host reconciliation pauses it whenever clarification, plan consent, approval, or activation needs a person. A delivery Goal cannot be marked complete before the Workbench reports Live or Stopped. Successful activation and explicit Stop Development reconcile the Goal without moving lifecycle authority into the Goal service.
