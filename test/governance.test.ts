...
function seeded(runtime = new InMemoryActivationRuntime()) {
  ...
  const governance = new GovernanceService(registry, workspace, runtime)
  const human = governance.issueAuthority({ kind: 'human-control', source: 'application-ui' })
  return { registry, workspace, governance, human, runtime }
}
...
describe('extension governance and recovery', () => {
  it('A. denies activation without approval', () => {
    ...
    assert.throws(() => governance.activate(candidate.id, human), ActivationDeniedError)
  ...
  it('F. upgrades an owner transactionally after trusted approval', () => {
    const { registry, workspace, governance, human } = seeded()
    const candidate = ready(workspace)
    const fingerprint = governance.requestApproval(candidate.id).fingerprint
    governance.recordApproval(human, { candidateId: candidate.id, fingerprint, decision: 'approved-for-exact-diff' })
    ...
    const after = governance.activate(candidate.id, human)
  ...
  it('G. rolls back a failed health check before commit', () => {
    const runtime = new InMemoryActivationRuntime()
    runtime.failHealth = true
    const { registry, workspace, governance, human } = seeded(runtime)
    const candidate = ready(workspace)
    const fingerprint = governance.requestApproval(candidate.id).fingerprint
    governance.recordApproval(human, { candidateId: candidate.id, fingerprint, decision: 'approved-for-exact-diff' })
    const status = governance.activate(candidate.id, human)
  ...
  it('H. restores the previous LKG after a committed activation', () => {
    const { registry, workspace, governance, human } = seeded()
    const candidate = ready(workspace)
    const fingerprint = governance.requestApproval(candidate.id).fingerprint
    governance.recordApproval(human, { candidateId: candidate.id, fingerprint, decision: 'approved-for-exact-diff' })
    governance.activate(candidate.id, human)
    const restored = governance.rollback(human)
  ...
  it('I. enters Safe Mode without the generated extension', () => {
    const { registry, workspace, governance, human } = seeded()
    const candidate = ready(workspace, {
    ...
    const safe = governance.enterSafeMode(human)
    ...
    governance.recordApproval(human, { candidateId: again.id, fingerprint: fp, decision: 'approved-for-exact-diff' })
  ...
  it('J. rejects attempts to rewrite the recovery root', () => {
    const { governance } = seeded()
    assert.throws(() => governance.rewriteRecoveryRoot(), GovernanceAuthorityError)
    assert.throws(() => governance.issueAuthority({ kind: 'assistant' } as never), GovernanceAuthorityError)
  })

  it('K. never leaves two active owners after a prepare failure', () => {
    const runtime = new InMemoryActivationRuntime()
