...
describe('TARS-NG mission-control workspace', () => {
  ...
  it('D. calendar create is a first-class approval object', () => {
    ...
    assert.equal(card?.sideEffect, 'create one event in an external calendar')
    assert.equal(card?.decision?.approveLabel, 'CREATE EVENT')
    assert.match(card?.decision?.reason ?? '', /external calendar/)
  ...
  it('projects an exact Obsidian write as a reviewable approval card', () => {
    ...
    assert.equal(card?.decision?.risk, 'local-write')
    assert.equal(card?.decision?.approveLabel, 'APPEND NOTE')
    assert.match(card?.decision?.scope ?? '', /configured vault/)
    assert.match(card?.details.join('\n') ?? '', /Use \[\[Governed Tools\]\]/)
    assert.match(card?.details.join('\n') ?? '', /abc123/)
  })
