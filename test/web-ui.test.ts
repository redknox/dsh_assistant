...
`
  ...
  it('keeps essential WUI text readable and narrow-screen Memory reachable', () => {
    ...
    assert.match(app, /className="conversation-empty"/)
    assert.match(app, /aria-label="Compact workspace"/)
    assert.match(app, /className="control-lamp" aria-hidden="true"/)
    assert.doesNotMatch(app, /ShuttlePrototypeSwitcher|prototype=shuttle|prototypeVariant/)
  })
})
